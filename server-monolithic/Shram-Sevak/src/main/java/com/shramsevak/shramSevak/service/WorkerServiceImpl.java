...
public class WorkerServiceImpl implements WorkerService {
	...
	public List<WorkerResponceDto> getAllWorkers(int pageNumber, int pageSize) {
		...
		List<WorkerResponceDto> workerDtoList = workerList.stream()
				.map(worker -> mapper.map(worker, WorkerResponceDto.class)).collect(Collectors.toList());
		if(workerDtoList.isEmpty())
			throw new ResourceNotFoundException("No worker found");
